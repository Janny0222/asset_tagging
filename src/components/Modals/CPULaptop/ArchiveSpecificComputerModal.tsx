...
const ArchiveSpecificComputerModal = ({ modalOpen, setModalOpen, onSubmit, id }: ChildrenModalProps) => {
    ...
    const handleSubmit = async () => {
        try {
            if( status === 'active') {
                const response = await archiveComputerInventory(id);
                setMessage('Successfully archived this data!')
                setModalOpen(false)
                computerInventoryListRefresh() 
            } else {
                const response = await reArchivedComputerInventory(id);
...
export default ArchiveSpecificComputerModal