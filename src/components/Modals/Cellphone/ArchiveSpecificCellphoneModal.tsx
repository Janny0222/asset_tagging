...
import { archiveCellphoneInventory, reArchiveCellphoneInventory } from '@/services/Cellphone/cellphoneService'
import { useMessage } from '@/context/MessageContext'
import { useStatusToggle } from '@/context/ToggleContext'
import { useCellphone } from '@/context/CellphoneContext'

const ArchiveSpecificCellphone = ({ modalOpen, setModalOpen, id }: ChildrenModalProps) => {
    ...
    const handleSubmit = async () => {
        try {
            if( status === 'active') {
                const response = await archiveCellphoneInventory(id);
                setMessage('Successfully archived this data!')
                
                cellphoneInventoryListRefresh() 
                setModalOpen(false)
            } else {
                const response = await reArchiveCellphoneInventory(id);
                setMessage('Successfully re-archived this data!')
...
export default ArchiveSpecificCellphone